...
def lnprob_nsamev(p, stars):
    ...
    for d, star in zip(p[:-4], stars):
        ...
        Xm = A.dot([1./d, vx, vy, vz])[:3]  # throw away RV
        D = np.array([p, pmra, pmdec])
        ll.append( lnprior_distance_constdens(d)  + lnlikelihood_gaussian(Xm, D, C) )
...
class SingleStar(object):
    ...
    def __init__(self, star, nwalkers=10, nsteps=5000, nburn=50):
        ...
        sampler = emcee.EnsembleSampler(nwalkers, SingleStar.ndim, SingleStar.lnprob, args=(star,))
...
class EnsembleFitter(object):
    ...
    def __init__(self, stars, nwalkers=10, nsteps=5000, nburn=50):
        ...
        #     + [np.random.normal(0, 30, nwalkers), np.random.normal(0, 30, nwalkers), np.random.normal(0, 30, nwalkers),
        #        np.random.uniform(0.,10.,nwalkers)]).T

        p0 = np.vstack(
            [np.random.normal(1./star.parallax.value, (star.parallax_error/star.parallax**2).value, nwalkers) for star in stars] \
            + [np.random.normal(10, 5, nwalkers), np.random.normal(45, 2, nwalkers), np.random.normal(0, 30, nwalkers),
               np.random.uniform(0.,10.,nwalkers)]).T
        ndim = p0.shape[1]

        sampler = emcee.EnsembleSampler(nwalkers, ndim, EnsembleFitter.lnprob, args=(stars,))
